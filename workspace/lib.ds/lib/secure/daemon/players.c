...
void validate(){
    //TODO fix base_name so the validations work
...
void AddPlayerInfo(mixed arg) {
   ...
   write_file("log_gab", "players.c start AddPlayerInfo "+arg+"\n");
    ...
    if(objectp(arg)){
        ...
        arg = lower_case(arg);
        write_file("log_gab", "players.c start string "+arg+"\n");
    ...
    if(grepp(player_save_file, DIR_CRES) && member_array(namestr,creators) == -1) 
        ...
        players += ({ namestr }) ; 
	//TODO why should the file exist at this point?	
    write_file("log_gab", "players.c before file exists "+player_save_file+"\n");

    if(!file_exists(player_save_file)) return;
    if(member_array(namestr,user_list) == -1)
        user_list += ({ replace_string(last_string_element(player_save_file,"/"),".o","") }) ; 
    write_file("log_gab", "players.c start Before save_object");
    //unguarded((: save_object, SAVE_PLAYER_LIST :));
	//does unguarded work at al???
