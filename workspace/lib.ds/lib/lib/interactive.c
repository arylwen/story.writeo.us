...
int Setup(){
    ...
    LoginTime = time();
    SetId(({}));
	write_file("log_gab", "interactive.c setup before autosave "+GetKeyName()+"\n");
    autosave::Setup();
...
varargs string array SetId(string *bogus){
    ...
    if(sizeof(bogus)){
        if(this_player() && this_player() == this_object()){
            //TODO check this if(!arrayp(bogus)) bogus = ({ bogus });
...
string GetShort(){
    ...
    if( !str ) TODO where is the method?*/
	str = "$N the unaccomplished";
