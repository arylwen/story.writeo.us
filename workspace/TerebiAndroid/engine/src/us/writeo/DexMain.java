...
public class DexMain {
    ...
    private static boolean processAllFiles() {
        outputDex = new DexFile(new DexOptions());
    ...
    private static boolean processOne(String pathname) {
        ...
        opener = new ClassPathOpener(pathname, false,
			new ClassPathOpener.Consumer() {
				...
				public boolean processFileBytes(String name, long p2, byte[] bytes)
				{
					return DexMain.processFileBytes(name, 0, bytes);
				}
     ...
     * @return whether processing was successful
     */
    private static boolean processFileBytes(String name, long flag, byte[] bytes) {
    ...
    private static boolean processClass(String name, byte[] bytes) {
        ...
        try {
            ClassDefItem clazz =
                CfTranslator.translate(name, bytes, args.cfOptions, null);
            outputDex.add(clazz);
            return true;
