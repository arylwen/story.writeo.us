...
private static void load_access(string cfg, mapping resource) {
    ...
    int valid_read(string file, object ob, string fun) {
        ...
        if( ob == master() ) return 1;
		//TODO see why this function doesn't work always
    ...
    mixed apply_unguarded(function f) {
        ...
        if(bbase_name(previous_object(0)) != SEFUN) {
	...
	string gbn_message;
	//aelyah - base_name is not called....
	string bbase_name(mixed val) {
        string name, base;
        int borg;
    ...
    varargs object player_object(string nom, object stub) {
		...
		ob = previous_object();
        tmp = bbase_name(ob);
		//string gbn = query_gbn_message();
		//error("base name is "+tmp);
