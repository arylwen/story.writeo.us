...
static string *titles = ({ "Apprentice Alchemist", "Alchemist", "Master Alchemist", });
...
query_guild_keep_player(object ob)
{
	...
	ob->catch_msg(why);
        ...
        set_alarm(1.0, 0.0, remove_craft_member);
...
{
    return MIN(MAX_GUILD_LEVEL, query_shadow_who()->query_stat(SS_CRAFT) / 50);
}
...
{
    int ret = shadow_me(who, "craft", GUILD_STYLE, GUILD_NAME);
